...
'%".get('q')."%'"; } 
        $hasil = $koneksi->prepare("SELECT * FROM sekolah ".$addonq." ORDER BY id DESC"); $hasil->execute(); 
        $data = $hasil->fetchAll(); 
?> 
...
class="pull-right"> 
        <form action="index.php?p=sekolah&m=search"> 
            ...
            <input autocomplete="off" type="text" name="q" placeHolder="Type and enter to search" value="
            <?php echo (get("q"));?>"> 
        </form> 
</div> 
